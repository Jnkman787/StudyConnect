...
import { collection, onSnapshot, getDocs, addDoc, setDoc, doc } from 'firebase/firestore';
...
const MessagesScreen = ({ navigation, route }) => {   // <-- pass the member data and use this to identify what to display
    ...
    useEffect(() => {
        ...
        // activate a listener to get realtime updates of the messages in the chatroom
        setupListener();
    }, []);
    ...
    // check whether or not a document exists in my direct-messages collection with a name = to the member.userID
    async function checkDocuments () {
        const colRef = collection(db, 'profiles', auth.currentUser.uid, 'direct-messages');
        const docsSnap = await getDocs(colRef);

        // look for the member's ID in the documents' ID
        for (let i in docsSnap.docs) {
            if (docsSnap.docs[i].id === member.userID) {
                return true;    // <-- a document has been found with the member's ID
            }
        }
        return false;   // <-- no document exists with the member's ID
    };

    // update listed messages
    async function setupListener () {
        let documentExists = await checkDocuments();

        if (!documentExists) {  // <-- if I need to add a new document
            const docData = {
                email: member.email,
            ...
            // start by adding a new document named after the member's ID
            setDoc(doc(db, 'profiles', auth.currentUser.uid, 'direct-messages', member.userID), docData)
            .then(docRef => {
                // then add a messages sub-collection (with its own document) inside the new created document
                let newDate = new Date();
                addDoc(collection(db, 'profiles', auth.currentUser.uid, 'direct-messages', member.userID, 'messages'), {
                    createdAt: newDate,
        ...
        onSnapshot(collection(db, 'profiles', auth.currentUser.uid, 'direct-messages', member.userID, 'messages'), docsSnap => {
            let messages = [];
    ...
    function addMessage () {
        ...
        addDoc(collection(db, 'profiles', auth.currentUser.uid, 'direct-messages', member.userID, 'messages'), docData)
        .then(docRef => {})
        .catch(error => console.log(error))
    ...
    return (
        ...
        >
            ...
            {addButton && <View style={styles.messageContainer}>
